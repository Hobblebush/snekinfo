defmodule SnekinfoWeb.UserAuthTest do
  ...
  @remember_me_cookie "_snekinfo_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SnekinfoWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{user: user_fixture(), staff: user_fixture(%{staff?: true}), conn: conn}
  end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SnekinfoWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_staff/2" do
    test "redirects if user is not authenticated", %{conn: conn} do
      conn = conn |> fetch_flash() |> UserAuth.require_authenticated_staff([])
      assert conn.halted
      assert redirected_to(conn) == "/"
